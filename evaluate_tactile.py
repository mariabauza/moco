...
def evaluate(train_loader, model, criterion, path_data, args):
    ...
    for i, images in enumerate(train_loader):
        ...
        acc1, acc5 = accuracy(output, target, args, topk=(1, 5), visualize = path_data)
    ...
    with torch.no_grad():
        ...
        if visualize is not None:
            ...
            if args.is_test:
                ...
                import importlib
                sensor = importlib.import_module('sensors.{}.sensor_params'.format(args.sensor_name))
                object_3D = Object3D(args.object_name, sensor, False, False)
            else:
            ...
            for i in range(batch_size):
                ...
                plt.imshow(np.concatenate([query, prediction], axis=1))
                plt.savefig(saving_path +'{}_correct={}.png'.format(target[i],correct[0,i]) ); plt.close()
                
                if args.is_test:
                    
                    transformation_real = np.load(path_query.replace('png', 'npy').replace('true_LS', 'trans'))
                    transformation_real[2,3] *=-1
                    transformation = np.load(path_pred.replace('0.png', 'transformation.npy'))
                    
                    dist = object_3D.poseDistance(Transformation(transformation), Transformation(transformation_real))
                    dists.append(dist)
                    plt.imshow(np.concatenate([query, prediction], axis=1))
