...
def plot_violin_v2(object_name, random, closest, best1, ICP1, best10,ICP10, figname):
  ...
  print('Dataset columns:', full_df.columns)
  ...
  pr = plt.plot(np.arange(Nstart,Nstop,0.1), [np.mean(random)*random_mean]*np.ones(len(N)), 'k', linewidth=5, solid_capstyle='round')
  pc = plt.plot(np.arange(Nstart,Nstop,0.1), [np.mean(closest)*random_mean]*np.ones(len(N)), '#C44E52', linewidth=5, solid_capstyle='round')
